...
#define chdir_xdg(xdg_name, xdg_fallback, dir)                                         \
    ({                                                                                 \
        chdir(getenv("HOME"));                                                         \
        ...
        mkdir(xdg, 0755);                                                              \
        if (chdir(xdg)) {                                                              \
            panic("Could not change the current directory to %s: %s", xdg,             \
                  strerror(errno));                                                    \
        }                                                                              \
        mkdir(dir, 0755);                                                              \
        if (chdir(dir)) {                                                              \
            panic("Could not change the current directory to %s/%s: %s", xdg, dir,     \
                  strerror(errno));                                                    \
        }                                                                              \
    })
